...
## 四、世界书 / 角色卡上下文（v1.3：勾选制侧边栏）
- **角色卡**：自动读取当前选中角色的 description/personality/scenario 字段。
- **世界书**（不再全量灌入！）载入后只是“登记条目”，真正发送哪些由你决定：
  1. 载入方式：点“导入世界书文件”选择世界书 JSON（如 命定之诗与黄昏之歌v4.3 (3).json），或“生成初稿”时自动探测酒馆当前激活世界书；
  2. 悬浮窗**左侧 ☰ 侧栏**可展开/收起：列出全部世界书条目（注释/标签/字符数/常驻标记），支持 搜索过滤、全选、仅常驻、清空勾选；
  3. 默认只勾选 **constant 常驻**条目（通常很少），其余请按开局需求自行勾选——**只有勾选的条目才会发给 AI**；
  4. 开关“带世界书”控制整块是否发送，“仅常驻”控制发送时是否过滤为非常驻条目，“注入上限”仍作为兜底截断。
> ⚠️ 直连主 API 模式不会执行世界书里的 EJS / MVU 脚本（这是选择该模式时已知的取舍），悬浮窗只负责“创作内容 + 按新格式汇总 JSON”。
## 五、已确认的依据（版本相关事实，非猜测）
