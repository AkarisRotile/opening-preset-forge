> v1.3.3：「地区·地理」按国家/大地区（方括号头段）二级分组：先出地区索引（可再展开），按名称排序并显示条数与已勾数。

> v1.3.2：按你手动清单校准 —— 组织·势力=12条指定条目、经济·产业=15条指定条目 全部命中；标题例外优先于关键词，其它归类不受影响。

> v1.3.1：世界书“其他”再梳理 —— 新增板块：导览开关 / 组织·势力 / 经济·产业 / 节庆；地区识别扩充（伯伦斯法环/萨赫拉联邦/永夜盟约/赛瑞利亚/冒险区域等）；规则词扩充。对 v4.3 其余条目归零。

> v1.3.0：世界书板块细分 —— DLC角色并入“角色”，DLC扩展/DLC事件单独板块；支持一次追加导入多本世界书（同文件重导=刷新，勾选记忆），[DLC][角色/扩展/事件] 前缀自动归类。
