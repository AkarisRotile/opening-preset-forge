...
## 工作流（Agent 式 · 单线程）
初稿生成后会自动进入“审稿”环节：
1. 浮窗出现 3~4 条**可选精修方向**（由主 API 根据初稿+需求生成，只发一次请求），点击任一条即精修；
2. 也可在下方输入框**自填方向**（类似 Agent 指令），点“按输入精修”；
3. 精修是**串行**的：在同一会话线程里追加“方向→修订稿”，**不会并发多次 API 请求**，可多轮迭代（每轮上限 6 次，超了会提示先导出/重开）；
4. 每轮修订后仍可“再给几组方向”换新方向，最终满意后一键导出。

生成与精修时都会注入 **《世界规则·创作限制》**（提炼自世界书《角色生成》《角色辅助指导》与 装备/道具/技能/资产 之书等条目）：
属性公式与 25 基础点/每级 1 点/层级点、开局最高 10 级与层级极值、品级-点数消耗、装备不改属性、资产与伙伴完整字段等，防止乱写。
